		...
		wn.bestFit(synsets[i], function (err, json) {
				if (err) {
					...
					if (json.lin.schema_dot_org) {
						...
						result += '\tlin: ' + json.lin.schema_dot_org + '\n';
					} else {
						numLinNone += 1;
						result += '\tlin: None\n';
					}
					if (json.rec.schema_dot_org) {
						...
						result += '\trec: ' + json.rec.schema_dot_org + '\n';
					} else {
						numRecNone += 1;
						result += '\trec: None\n';
					}
					if (json.rec.schema_dot_org !== json.lin.schema_dot_org) {
						different += 1;
						result += 'Different\n';
						fs.appendFile('testing/compare-schema_dot_org-different', result);
					} else {
						same += 1;
						result += 'Same\n';
						fs.appendFile('testing/compare-schema_dot_org-same', result);
					}
					console.log('Wrote result for ' + json.synset);
					fs.appendFile('testing/compare-schema_dot_org', result, function () {
						numSynsets += 1;
						writeSense(i+1);
					});
				}
			...
			console.log(stats);
			fs.appendFile('testing/compare-schema_dot_org-stats', stats);
		}
	};
	writeSense(index);
